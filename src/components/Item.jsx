...
function Item({ title, price, image, id, des, rating }) {
    ...
    return (
        <div
            ...
            <div className='flex items-center w-full justify-between h-full p-2'>
                <h1 className='max-w-[10rem] h-auto font-bold'>{title}</h1>
                <h1 className='text-md'>${price}</h1>
            </div>
            <div className='self-start ml-3 mb-4'>
                Rating: <span className='text-xl text-yellow-500'>{rating}</span>
            </div>
            <div className='flex justify-evenly items-center w-full'>
                ...
                <button
                    className='text-primary font-bold border-[1px] border-primary bg-white border-opacity-40 hover:opacity-90 w-[7rem] rounded p-2 mb-3'
                    onClick={(e) => {
                        e.stopPropagation();
                        dispatch(addItem({
                            id,
